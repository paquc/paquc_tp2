...
if len(sys.argv) > 5:
    ...
    prediction_window = int(sys.argv[5])
    sliding_window = int(sys.argv[6])
    use_bootstrap = int(sys.argv[7])
    n_bootstrap_samples = int(sys.argv[8])
    train_data_size = int(sys.argv[9])
    test_data_size = int(sys.argv[10])
    val_data_size = int(sys.argv[11])
else:
    print("Usage: Train.py <data_set_id>")
    sys.exit(1)

# Node name for files ONLY!
node_name_formated = node_name.replace(':', '-')

file_suffix = f"{node_name_formated}_{window_size}_{prediction_window}_{sliding_window}"    

# Ex. KERNDTLB_alarm_occurences_matrix_R30-M0-N9-C-J16-U01_1800_600_350_chrono_dedup
# Charger la matrice d'occurrence
input_file_path = f"./BGL_Brain_results/KERNDTLB_alarm_occurences_matrix_{file_suffix}_chrono_dedup.csv"
...
with open(f"./BGL_Brain_results/KERNDTLB_Training_Set_RF_{sampling_data_desc}_{file_suffix}_Output.log", "w") as RF_log_file, open(f"./BGL_Brain_results/KERNDTLB_Training_Set_LR_{sampling_data_desc}_{file_suffix}_Output.log", "w") as LR_log_file:

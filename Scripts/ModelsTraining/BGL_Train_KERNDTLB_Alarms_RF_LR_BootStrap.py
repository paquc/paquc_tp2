...
#data = pd.read_csv('./BGL_Brain_results/KERNDTLB_alarm_occurences_matrix_V1_part1_dedup.csv')
...
with open("./BGL_Brain_results/BGL_KERNDTLB_Trainig_Output.log", "w") as log_file:
    for sample_index in range(n_data_samples):
        
        data = pd.read_csv(f"./BGL_Brain_results/KERNDTLB_alarm_occurences_matrix_V1_part{sample_index+1}_dedup.csv")
        msg=f"--- Sample Data part {sample_index+1}/{n_data_samples} ---"
        print(msg)
        log_file.write(msg+"\n")

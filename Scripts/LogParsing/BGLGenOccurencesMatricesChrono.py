...
if len(sys.argv) >= 4:
    ...
    time_window_epoch = int(sys.argv[2])
    prediction_window_epoch = int(sys.argv[3])
    moving_window_epoch = int(sys.argv[4])
else:
...
def print_alarm_types(df_logs, node_name = 'R30-M0-N9-C:J16-U01', alarm_name = 'KERNDTLB'):
    ...
    with open(f"./BGL_Brain_results/{node_name_formated}_alarms_types.csv", "w") as alarm_tpes_output_file:
...
def GenMatrices():
    ...
    with open(f"./BGL_Brain_results/{alarm_name}_alarm_sequences_{node_name_formated}_{time_window_epoch}_{prediction_window_epoch}_{moving_window_epoch}_chrono.csv", "w") as sequences_output_file:
        ...
        # print_alarm_types(df_logs, node_name)
        # sys.exit(0)
